...
public interface DailyStatsRepository extends JpaRepository<DailyStats, String> {
    ...
    @Modifying
    @Query("UPDATE DailyStats d SET " +
           "d.averageBurdenScore = ((d.averageBurdenScore * d.submissionCount) + :newScore) / (d.submissionCount + 1), " +
           "d.submissionCount = d.submissionCount + 1, " +
           "d.lastUpdated = CURRENT_TIMESTAMP " +
           "WHERE d.countryCode = :countryCode")
    int incrementSubmissionCountAndScore(@Param("countryCode") String countryCode, @Param("newScore") Double newScore);
}
